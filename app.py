...
def fetch_from_github_requests():
    try:
        ...
        # إذا لم يكن الملف موجوداً بعد، ننشئ ملف جديد
        create_initial_file()
        return True

def create_initial_file():
    """إنشاء ملف Excel أولي بالأعمدة المطلوبة"""
    try:
...
def add_new_machine(df, machine_name, lubrication_type, interval_days):
    """إضافة ماكينة جديدة مع تحديد نوع التشحيم والمدة"""
...
def update_daily_counters(df):
    """تحديث العدادات اليومية بناءً على التاريخ الحالي"""
    if df.empty:
        return df
...
def get_overdue_machines(df):
    """الحصول على الماكينات المتأخرة عن موعد التشحيم"""
    if df.empty:
        return pd.DataFrame()
...
def get_upcoming_machines(df, days=7):
    """الحصول على الماكينات التي ستنتهي خلال عدد محدد من الأيام"""
    if df.empty:
        return pd.DataFrame()
...
def show_dashboard(df):
    """عرض لوحة التحكم مع العدادات التنازلية"""
    ...
    # عرض الماكينات التي ستنتهي قريباً
    ...
    # جدول جميع الماكينات مع العدادات
...
def show_machine_details(df):
    ...
    if machine_name:
        ...
        # سجل التشحيم للماكينة (يمكن إضافة تاريخ سابق في المستقبل)
        st.subheader("📋 سجل التشحيم")
        st.info("سيتم عرض سجل عمليات التشحيم السابقة هنا قريباً")
