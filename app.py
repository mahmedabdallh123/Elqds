...
def search_across_sheets(all_sheets, equipment_config):
    """البحث المتقدم - يمكن البحث في شيت محدد أو جميع الشيتات، مع فلتر التاريخ"""
...
if can_edit and len(tabs) > 1:
    with tabs[1]:
        ...
        sheets_edit = manage_data_edit(sheets_edit, equipment_config)import streamlit as st
import pandas as pd
import json
import os
import io
...
def search_across_sheets(all_sheets, equipment_config):
    ...
    if st.button("🔍 بحث", key="search_btn", type="primary"):
        ...
        # تحديد الشيتات المراد البحث فيها
        sheets_to_search = all_sheets.items()
        if selected_sheet != "جميع الشيتات":
            sheets_to_search = [(selected_sheet, all_sheets[selected_sheet])]
...
# تبويبات التطبيق الرئيسية (تم حذف تبويب عرض البيانات المنفصل)
tabs_list = ["🔍 بحث متقدم"]
...
APP_CONFIG = {
    ...
    "EXPECTED_COLUMNS": {
        "equipment": ["equipment", "machine", "معدة", "ماكينة", "جهاز", "Equipment", "Machine"],
        "date": ["date", "تاريخ", "time", "وقت", "Date", "DATE"],
        "event": ["event", "حدث", "issue", "مشكلة", "Event", "الحدث"],
        "correction": ["correction", "تصحيح", "solution", "حل", "Correction", "التصحيح"],
        "servised_by": ["servised", "serviced", "service", "technician", "فني", "تم بواسطة"],
        "tones": ["tones", "طن", "أطنان", "ton", "tone", "Tones"],
        "images": ["images", "pictures", "صور", "مرفقات", "Images"]
    }
}
...
def add_new_event_with_equipment(sheets_edit, sheet_name, equipment_list, unique_id):
    ...
    with col1:
        selected_equipment = st.selectbox("🔧 اختر المعدة:", equipment_list, key=f"eq_select_{unique_id}")
        event_date = st.date_input("📅 التاريخ:", value=datetime.now(), key=f"date_{unique_id}")
        event_desc = st.text_area("📝 الحدث/العطل:", height=100, key=f"event_{unique_id}")
    
    with col2:
        correction_desc = st.text_area("🔧 التصحيح:", height=100, key=f"correction_{unique_id}")
        servised_by = st.text_input("👨‍🔧 تم بواسطة:", key=f"servised_{unique_id}")
        tones = st.text_input("⚖️ الأطنان:", key=f"tones_{unique_id}")
    ...
    notes = st.text_area("📝 ملاحظات:", key=f"notes_{unique_id}")
...
def manage_sheet_equipment(sheet_name, config, unique_id):
    ...
    with col1:
        st.markdown("#### ➕ إضافة معدة")
        new_equipment = st.text_input("اسم المعدة:", key=f"new_eq_{unique_id}")
        if st.button("➕ إضافة", key=f"add_eq_{unique_id}"):
            if new_equipment:
                success, msg = add_equipment_to_sheet(sheet_name, new_equipment, config)
                if success:
...
def display_sheet_data_with_filter(sheet_name, df, equipment_list, unique_id):
    st.markdown(f"### 📄 {sheet_name}")
    st.info(f"عدد الصفوف: {len(df)} | الأعمدة: {len(df.columns)}")
...
def search_across_sheets(all_sheets, equipment_config):
    ...
    with col2:
        ...
        filter_equipment = st.selectbox("🔧 فلتر حسب المعدة:", ["الكل"] + all_equipment, key="search_eq")
    with col3:
        search_in_column = st.selectbox("📋 البحث في عمود:", ["الكل", "Equipment", "Event", "Correction", "Notes"], key="search_col")
    ...
    if st.button("🔍 بحث", key="search_btn", type="primary"):
        ...
        if results:
            ...
            st.success(f"✅ تم العثور على {len(combined)} نتيجة")
            st.dataframe(combined, use_container_width=True, height=500)
            csv = combined.to_csv(index=False).encode('utf-8')
            st.download_button("📥 تحميل CSV", csv, f"search_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv", "text/csv")
        else:
...
def add_new_sheet_to_github(sheets_edit, equipment_config, unique_id):
    ...
    col1, col2 = st.columns(2)
    with col1:
        new_sheet_name = st.text_input("اسم الشيت الجديد:", key=f"new_sheet_{unique_id}")
        if new_sheet_name and new_sheet_name in sheets_edit:
            st.warning(f"⚠ الشيت موجود بالفعل")
    with col2:
        default_columns = st.text_area("الأعمدة:", value="\n".join(APP_CONFIG["DEFAULT_SHEET_COLUMNS"]), key=f"cols_{unique_id}", height=150)
...
def manage_data_edit(sheets_edit, equipment_config):
    ...
    tab_names = ["📋 عرض وإدارة البيانات", "➕ إضافة حدث جديد", "🔧 إدارة المعدات", "➕ إضافة شيت جديد", "📷 إدارة الصور"]
    tabs_edit = st.tabs(tab_names)
    ...
    with tabs_edit[0]:
        ...
        if sheets_edit:
            ...
            for i, (sheet_name, df) in enumerate(sheets_edit.items()):
                with sheet_tabs[i]:
                    equipment_list = get_sheet_equipment(sheet_name, equipment_config)
                    display_sheet_data_with_filter(sheet_name, df, equipment_list, f"{edit_unique_id}_{sheet_name}_view")
                    
                    with st.expander("✏️ تعديل مباشر"):
                        edited_df = st.data_editor(df.astype(str), num_rows="dynamic", use_container_width=True, key=f"editor_{edit_unique_id}_{sheet_name}")
...
view_unique_id = str(uuid.uuid4())[:8]
...
with tabs[0]:
    ...
    if all_sheets is None:
        ...
        for i, (sheet_name, df) in enumerate(all_sheets.items()):
            with sheet_tabs[i]:
                equipment_list = get_sheet_equipment(sheet_name, equipment_config)
                display_sheet_data_with_filter(sheet_name, df, equipment_list, f"{view_unique_id}_{sheet_name}")
