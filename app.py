...
if can_edit and len(tabs) > 1:
    with tabs[1]:
        sheets_edit = manage_data_view_only(sheets_edit)import streamlit as st
import pandas as pd
import numpy as np
import json
import os
...
# ==================== دوال إضافة الشيت فقط ====================
def add_new_sheet_only(sheets_edit):
...
def manage_data_view_only(sheets_edit):
    ...
    # تبويبان فقط: عرض البيانات + إضافة شيت جديد
    tab_names = ["📋 عرض البيانات", "➕ إضافة شيت جديد"]
    tabs_view = st.tabs(tab_names)
    
    with tabs_view[0]:
        st.subheader("جميع الشيتات (عرض فقط)")
        if sheets_edit:
            sheet_tabs = st.tabs(list(sheets_edit.keys()))
            for i, (sheet_name, df) in enumerate(sheets_edit.items()):
                with sheet_tabs[i]:
            ...
            st.info("لا توجد شيتات لعرضها")
    
    with tabs_view[1]:
        sheets_edit = add_new_sheet_only(sheets_edit)
...
# تبويب واحد فقط للعرض، وتبويب الإدارة سيظهر فقط للمستخدمين المصرح لهم ويحتوي على إضافة شيت فقط
tabs_list = ["📊 عرض البيانات"]
if can_edit:
    tabs_list.append("🛠 إدارة التعديلات (إضافة شيت فقط)")
