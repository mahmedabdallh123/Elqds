...
EQUIPMENT_CONFIG_FILE = "equipment_config.json"  # ملف لحفظ تكوينات المعدات لكل شيت
...
def add_new_event_with_equipment(sheets_edit, sheet_name, equipment_list):
    ...
    # إنشاء نموذج الإدخال مع قائمة المعدات
    col1, col2 = st.columns(2)
    
    with col1:
        selected_equipment = st.selectbox("🔧 اختر المعدة:", equipment_list, key=f"equipment_select_{sheet_name}_{datetime.now().timestamp()}")
        event_date = st.date_input("📅 التاريخ:", value=datetime.now(), key=f"date_{sheet_name}_{datetime.now().timestamp()}")
        event_desc = st.text_area("📝 الحدث/العطل:", height=100, key=f"event_{sheet_name}_{datetime.now().timestamp()}", placeholder="وصف العطل أو المشكلة...")
    
    with col2:
        correction_desc = st.text_area("🔧 التصحيح/الإجراء المتخذ:", height=100, key=f"correction_{sheet_name}_{datetime.now().timestamp()}", placeholder="ما الذي تم إصلاحه أو الإجراء الذي تم...")
        servised_by = st.text_input("👨‍🔧 تم بواسطة (الفني):", key=f"servised_{sheet_name}_{datetime.now().timestamp()}", placeholder="اسم الفني أو المشغل")
        tones = st.text_input("⚖️ الأطنان/الكمية:", key=f"tones_{sheet_name}_{datetime.now().timestamp()}", placeholder="مثال: 5.5 طن")
    ...
    uploaded_files = st.file_uploader(
        ...
        key=f"images_{sheet_name}_{datetime.now().timestamp()}"
    )
    ...
    # ملاحظات إضافية
    notes = st.text_area("📝 ملاحظات إضافية:", key=f"notes_{sheet_name}_{datetime.now().timestamp()}", placeholder="أي ملاحظات إضافية...")
...
def display_sheet_data_with_filter(sheet_name, df, equipment_list):
    ...
    if equipment_list and "Equipment" in df.columns:
        ...
        with filter_col1:
            # استخدام unique key لتجنب التكرار
            filter_key = f"filter_{sheet_name}_{datetime.now().timestamp()}"
...
def search_across_sheets(all_sheets, equipment_config):
    ...
    with col2:
        # جمع كل المعدات من جميع الشيتات
        all_equipment = []
...
def add_new_sheet_to_github(sheets_edit, equipment_config):
    ...
    with col_btn2:
        if st.button("✅ إنشاء وإضافة الشيت الجديد", key="create_sheet_btn", type="primary", use_container_width=True):
            ...
            if new_sheets is not None:
                # تهيئة تكوين المعدات للشيت الجديد
    ...
    if equipment_list:
        ...
        if st.button("💾 حفظ التعديل", key=f"save_eq_edit_{sheet_name}"):
            if new_eq_name:
                success, msg = update_equipment_in_sheet(sheet_name, eq_to_edit, new_eq_name, config)
                if success:
                    # تحديث البيانات في الشيت لتغيير اسم المعدة في جميع الصفوف
                    st.success(msg)
                    st.info("⚠ ملاحظة: سيتم تحديث اسم المعدة في السجلات المستقبلية فقط. السجلات السابقة ستبقى بالاسم القديم.")
                    st.rerun()
                else:
...
def display_sheet_data_with_filter(sheet_name, df, equipment_list):
    ...
    if equipment_list and "Equipment" in df.columns:
        ...
        with filter_col2:
            if st.button("🔄 عرض الكل", key=f"reset_filter_{sheet_name}"):
                st.session_state[f"filter_{sheet_name}"] = "جميع المعدات"
                st.rerun()
