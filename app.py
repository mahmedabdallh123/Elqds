...
def add_new_sheet(sheets_edit):
    ...
    with col1:
        new_sheet_name = st.text_input("اسم الشيت الجديد:", placeholder="مثال: بيانات_2025", key="new_sheet_name")
    
    with col2:
        num_columns = st.number_input("عدد الأعمدة:", min_value=1, max_value=50, value=5, key="num_columns")
    
    st.markdown("### 📋 تعريف الأعمدة")
    
    columns_data = []
    cols_display = st.columns(min(3, num_columns))
    
    for i in range(num_columns):
        with cols_display[i % 3]:
            col_name = st.text_input(f"اسم العمود {i+1}:", value=f"عمود_{i+1}", key=f"col_name_{i}")
            col_type = st.selectbox(f"نوع العمود {i+1}:", ["نص", "رقم", "تاريخ", "صور"], key=f"col_type_{i}")
            columns_data.append({"name": col_name, "type": col_type})
    
    if st.button("✨ إنشاء الشيت الجديد", key="create_new_sheet", type="primary"):
        ...
        for col_info in columns_data:
            if col_info["name"]:
                new_df_data[col_info["name"]] = []
        
        new_df = pd.DataFrame(new_df_data)
        
        # إضافة الشيت إلى القاموس
    ...
    if existing_sheets:
        ...
        if st.button("🗑️ حذف الشيت", key="delete_sheet_btn"):
            if sheet_to_delete:
                confirm = st.checkbox(f"تأكيد حذف الشيت '{sheet_to_delete}'؟ لا يمكن التراجع عن هذا الإجراء")
                if confirm:
        ...
        st.info("ℹ️ لا توجد شيتات للحذف")
